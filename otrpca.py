...
def otrpca(M, rank=1, initial_A='t_SVD', reg_l_1=None, reg_nuc=1, reg_c=100, bc_conv=5, bc_error=1e-8):
    ...
    reg_nuc : float, regularisation parameter on the frobenius norm that is applied to the low rank tensor (lamda_2 in Algorithm 1 of [1])
    		  defaults to 1

    reg_c : float, regularistion on the frobenius norm that is minimised so that M = L + C. So the larger it is, the larger the penalty is for
    		L + C differeing from M. In [1] this apears above equation 4.01 [1] as lambda_3. In the text, this value is divided out 
    		(so lambda_i -> lambda_i / lambda_3). This has not been done in this implementation to improve compuational stability.
    		defaults to 100

    bc_conv : integer, the number of times that the error in min_b_c bust be below bc_error for min_b_c to have converged	
    		  defaults to 5
...
def min_b_c(m, A, b, reg_c, reg_nuc, reg_l_1, frontal_faces, bc_conv=5, bc_error=1e-8):
	"""Finds the b and c that minimise equation 4.2.6 in [1] (this is algorithm 2 in [1])
    
    Parameters
    ----------
    m : np-dimensional array, slice t of M (fixing second axis, indexed 1) (m_t in algorithm 1 of [1])
	...
	while(small_errors < bc_conv):
	    ...
	    error = max(np.linalg.norm(c - c_minus_one), np.linalg.norm(b - b_minus_one))
	    
	    #while loop concludes when the error is below the given value bc_conv times
	    if(error < bc_error):
...
def min_A(A, X, Y, frontal_faces):
	"""Finds the A that minimises equation 4.2.6 in [1] (this is algorithm 3 in [1])
	...
	for j in range(dim[1]):
	    ...
	    for k in range(len(A_f)):
	        ...
	        #calculate new value
	        A_f_i[:,j] = (X_f_i[:,j] - A_f_i @ Y_f_i[:,j]) / Y_f_i[j,j] + A_f_i[:,j]
