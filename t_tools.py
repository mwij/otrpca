...
def get_frontal_faces(A):
    ...
    provides a list of the indices that need to be fixed in each
    dimension (other than the first two) to produce the base-case 
    matrices to be multiplied.
    
    This was previously within the t_prod function, but repeated 
    evaluation had a large computational cost that was not neccery.
    
    """
    
    dims = A.shape
    a_frontal_slice_index = tuple(
        [0,0] 
        + [slice(0, dims[i + 2]) for i in range(len(dims) - 2)])
...
def t_prod(A, B, frontal_faces=None):
    ...
    Product of two p-dimensional tensors, C = A*B, as defined in [1] and reproduced in Definition 2.4 in [2].
    ...
    SIAM Journal on Scientific Computing, vol. 35, n. 1, pp. 474-490, 
    2013. DOI: 10.1137/110841229
...
def t_SVD(A, frontal_faces=None):
    ...
    V : np-dimensional array (usually written transposed V^{T},
        similar to the SVD)
...
def t_transpose(A, frontal_faces = None):
    ...
    for index in frontal_faces:
        ...
        i_A = tuple([slice(0, dim_A[0]), slice(0, dim_A[1])] 
            + [index[i] for i in range(len(index))])
        i_A_T = tuple([slice(0, dim_A_T[0]), slice(0, dim_A_T[1])] 
            + [(-index[i] % (dim_A[i + 2])) for i in range(len(index))])
...
def tdiag_list(A, frontal_faces=None):
    ...
    ----------
    storage : list
        list of tuples containing all frontal faces (in fourier domain) 
        and that face's index
    """
...
def tdiag_list_inverse(A_f, dim):
    ...
    A_f : list
        list of tuples containing all frontal faces (in fourier domain) 
        and that face's index of a tensor A
